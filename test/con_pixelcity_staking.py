...
def depositRewards(mob_amount: float):
    ...
    for plot_thing in ForeignVariable(foreign_contract=pixelcity_contract.get(), foreign_name='valid_plot_nfts'):
        if(plots[plot_thing] != 0):
            if(plots[plot_thing]["built"] != ""):
                staking_rewards[plots[plot_thing]["current_build_owner"]] += mob_amount / (len(ForeignVariable(foreign_contract=pixelcity_contract.get(), foreign_name='in_use_building_nfts')))
